...
groupshared float2 sharedAOAndViewZ[THREAD_Y];
...
void main(int2 groupID : SV_GroupID, int2 threadID : SV_GroupThreadID)
{
	...
	const int thread = threadID.y;
	int localPos;
	int2 pixel;
	pixel.x = groupID.x;
	pixel.y = calSamplingPos(localPos, groupID.y, thread, THREAD_Y, cbParam.blurRadius);
	
	sharedAOAndViewZ[thread] = samplingValues(pixel, texSize, tAO, tViewZ);
	
	GroupMemoryBarrierWithGroupSync();
	
	[branch] if(cbParam.blurRadius <= thread && thread < (THREAD_Y - cbParam.blurRadius) && pixel.x < texSize.x && pixel.y < texSize.y) {
